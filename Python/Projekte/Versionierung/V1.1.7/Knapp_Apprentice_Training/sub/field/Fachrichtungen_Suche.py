...
class Suche_Dialog(QDialog, Ui_Suche_Dialog):
    ...
    def on_btSuchen_clicked(self):
        ...
        f = self.leSuche.text()
        if f != "":
            self.tw_Anzeigen_suche(f)
        else:
            self.tw_Anzeigen_suche
    
    def tw_Anzeigen_suche(self, f=""):
        ...
        if f != "":
            sql= "SELECT fachrichtung, bezeichnung, lehrjahre, benutzername, aenderungsdatum FROM KAT_fachrichtungen WHERE fachrichtung LIKE '%"+f+"%' OR bezeichnung LIKE '%"+f+"%' OR lehrjahre LIKE '%"+f+"%'"
        else :
    ...
    def on_twAnzeige_suche_cellClicked(self, row, column):
        self.parent().leFachrichtungabk.setText(self.twAnzeige_suche.item(row,  0).text())
        self.parent().leFachrichtungbez.setText(self.twAnzeige_suche.item(row,  1).text())
        self.parent().sbLehrjahre.setValue(float(self.twAnzeige_suche.item(row,  2).text()))
