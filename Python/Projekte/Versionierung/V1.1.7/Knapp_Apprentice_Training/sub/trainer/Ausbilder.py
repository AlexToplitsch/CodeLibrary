...
class Ausbilder(QDialog, Ui_Ausbilder):
    ...
    def cell_tw_anzeige(self):
        ...
        sql = "SELECT Familienname, Vorname, Telefon, Wochenstunden, Eintrittsdatum, Austrittsdatum, wt_mo, wt_di, wt_mi, wt_do, wt_fr, Änderung, Benutzer Vor FROM kat_ausbilder"
        mycursor = db.select(sql)
        if mycursor == "backtologin":
            self.back_to_login()
            return
