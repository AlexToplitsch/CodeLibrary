...
class Lehrstoff_Suche(QDialog, Ui_Lehrstoff_Suche):
    ...
    def on_tW_Lehrstoff_DB_cellClicked(self, row, column):
        ...
        """
        try:
            self.parent().cB_Ausbildungsthema.setCurrentText(self.tW_Lehrstoff_DB.item(row, 1).text())
            self.parent().le_Lehrstoff_ID.setText(self.tW_Lehrstoff_DB.item(row, 2).text())
            self.parent().le_Hauptthema.setText(self.tW_Lehrstoff_DB.item(row, 3).text())
            self.parent().cB_Verantwortlicher.setCurrentText(self.tW_Lehrstoff_DB.item(row, 4).text())
            self.parent().le_Lehrstoff.setText(self.tW_Lehrstoff_DB.item(row, 5).text())
            self.parent().le_Dokument.setText(self.tW_Lehrstoff_DB.item(row, 6).text())
            self.parent().sB_Stunden.setValue(int(self.tW_Lehrstoff_DB.item(row, 7).text()))
            self.parent().le_Hilfsmittel.setText(self.tW_Lehrstoff_DB.item(row, 8).text())
        except Exception:
    ...
    def tW_Lehrstoff_DB_Anzeigen(self, f1 = ""):
       ...
       #überschriften setzen
        ...
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(0, colname)
        colname = QTableWidgetItem("Lehrstoff_ID")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(1,  colname)
        colname = QTableWidgetItem("Hauptthema")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(2, colname)
        colname = QTableWidgetItem("Lehrstoff")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(3, colname)
        colname = QTableWidgetItem("Verantwortlicher")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(4, colname)
        colname = QTableWidgetItem("Dokument")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(5, colname)
        colname = QTableWidgetItem("Stunden")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(6, colname)
        colname = QTableWidgetItem("Hilfsmittel")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(7, colname)
        colname = QTableWidgetItem("LAP-Refernz-ID")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(8, colname)
        colname = QTableWidgetItem("Benutzer")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(9, colname)
        colname = QTableWidgetItem("Änderung")
        self.tW_Lehrstoff_DB.setHorizontalHeaderItem(10, colname)
        ...
        if f1 != "":
            sql = "select * from kat_lehrstoff where concat(Ausbildungsthema, Lehrstoff_ID, Hauptthema, Lehrstoff, Verantwortlicher, Dokument) like '"+ '%' + f1 + '%' "'"
        else:
        ...
        for z in mycursor:
            for s in range(0,  11):
                ...
                self.tW_Lehrstoff_DB.setItem(zeile,  s,  fielditem)
            zeile+=1
