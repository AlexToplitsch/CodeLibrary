...
class Suche_Dialog(QDialog, Ui_Suche):
    ...
    def on_tb_Raeume_cellClicked(self, row, column):
        ...
        self.cb_Bs.setCurrentText(self.tb_Raeume.item(row,  4).text())
        self.le_Ansprechpartner.setText(self.tb_Raeume.item(row,  5).text())
    ...
    def cell_tb_Raeume(self):
        ...
        self.tb_Raeume.setColumnCount(9)
        ...
        self.tb_Raeume.setHorizontalHeaderItem(8, colname) 
    
        
        self.tb_Raeume.setRowCount(1)
        sql = "SELECT * from KAT_berufsschulzeit WHERE PERS_NR LIKE '" + '%' + self.leRaeume.text() + '%' + "'"
