    ...
    def toSQLite(self):
        ...
        if 'kat.db' in os.listdir('.//'):
            try:
                ...
                pass
        source_path = r"\\10.33.35.171\APPs\Database"
        dest_path = os.getcwd()
        file_name = "\kat.db"

        #shutil.copyfile(source_path + file_name, dest_path + file_name)
        command = 'xcopy ' + source_path + file_name + ' ' + dest_path
        print(command)
        #subprocess.call(["xcopy", source_path + file_name, dest_path, "/I"])
        subprocess.Popen(command, shell=True)
        
        #command = 'python mysql_to_sqlite3\cli.py  "kat.db"  "' + self.__databasename +'"  "' + "root" +'"  "' + "knapp22" +'"  "' + self.__host +'"'
        #command = 'python mysql_to_sqlite3\cli.py "kat.db" "root" "knapp22" "kat" "10.33.35.171"'
        
        #command = 'mysql2sqlite -f "kat.db" -d "' + self.__databasename +'" -u "' + "root" +'" -p "' + "knapp22" +'" -h "' + self.__host +'" -q'
        #command ='python mysql_to_sqlite3\cli.py'
        #print(command)
        #subprocess.Popen(command, shell=True)
        
        #subprocess.Popen('python db.py')
