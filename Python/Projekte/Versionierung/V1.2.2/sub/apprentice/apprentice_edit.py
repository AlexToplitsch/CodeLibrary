...
class GuiApprenticeEdit(QDialog, Ui_ApprenticeEdit):
    ...
    def db_auslesen(self):
        ...
        for z in cursor: #Zeile auslesen und ausgeben
            ...
            self.lineEdit_vorname.setText(str(z[1]))
            self.lineEdit_nachname.setText(str(z[2]))
            self.spinBox_lehrdauer.setValue(float(z[9]))
    ...
    @pyqtSlot()
    def on_pushButton_change_picture_clicked(self):
    ...
    def on_pushButton_speichern_clicked(self):
        ...
        if len(errors) < 1:
            ...
            sql = "REPLACE INTO kat_lehrlinge (Personalnummer,Vorname,Nachname,Geburtsdatum,Geschlecht,Lehrberuf,Hauptmodul,Spezialmodul,Lehrbeginn,Lehrdauer,Mail,Telefonnummer,Strasse,Hausnummer,PLZ,ORT,LKZ,Bild) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
