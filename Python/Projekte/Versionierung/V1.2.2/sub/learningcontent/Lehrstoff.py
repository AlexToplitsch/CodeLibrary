...
class Lehrstoff(QDialog, Ui_Dialog):
    ...
    def on_pB_Speichern_clicked(self):
        ...
        """
        if self.cB_Ausbildungsthema.currentText() == "" or self.cB_Ausbildungsthema.currentText() == " ":
            self.INFO("Bei 'Ausbildungsthema' wurde nichts ausgewählt!",  "H")
            return
        if self.cB_Verantwortlicher.currentText() == "" or self.cB_Verantwortlicher.currentText() == " ":
            self.INFO("Bei 'Verantwortlicher' wurde nichts ausgewählt!",  "H")
            return
        if self.cB_Reference.currentText() == "" or  self.cB_Reference.currentText() == " ":
            self.INFO("Bei 'Referenz-ID' wurde nichts ausgewählt!",  "H")
            return
        ...
        try:
            ...
            if satz_len == 0:
                ...
                if self.lbl_Dokumentpfad.text() == "Dokument vorhanden!" or self.lbl_Dokumentpfad.text() == "Kein Dokument vorhanden!":
                    print("if")
                    sql = "UPDATE kat_lehrstoff set Hauptthema=%s, Lehrstoff=%s, Verantwortlicher=%s, Stunden=%s, Hilfsmittel=%s, LAPThemenID=%s, Benutzer=%s" + " WHERE Ausbildungsthema='" + self.cB_Ausbildungsthema.currentText() + "' AND Lehrstoff_ID='" + self.le_Lehrstoff_ID.text()+"'"
                    val = (self.le_Hauptthema.text(), self.le_Lehrstoff.text(), self.cB_Verantwortlicher.currentText(),  self.sB_Stunden.value(), self.le_Hilfsmittel.text(),  self.cB_Reference.itemData(self.cB_Reference.currentIndex()), self.parent().label_eingeloggt_als.text())
                else:
                    ...
                    val = (self.le_Hauptthema.text(), self.le_Lehrstoff.text(), self.cB_Verantwortlicher.currentText(), self.binaryData,  self.sB_Stunden.value(), self.le_Hilfsmittel.text(),  self.cB_Reference.itemData(self.cB_Reference.currentIndex()), self.parent().label_eingeloggt_als.text())
    ...
    def on_tW_Datenbank_cellClicked(self, row, column):
        ...
        try:
            ...
            while run == True:
                try:
                    i += 1
                    if self.tW_Datenbank.item(row, 8).text() == "0" or str(self.cB_Reference.itemData((i))) == "None":
                        self.INFO("In diesem Themenkatalog ist kein Referenzthema mit dieser ID bekannt! Wechseln Sie den Referenzfilter!",  "H")
                        return
                    if str(self.cB_Reference.itemData(i)) == self.tW_Datenbank.item(row, 8).text():
                        self.cB_Reference.setCurrentIndex(i)
                        self.INFO("Referenz ID wurde gefunden!",  "I")
                        return
                except Exception:
                    return
        except Exception as e:
    ...
    def on_pB_Dok_Anzeigen_clicked(self):
        ...
        try:
            if self.lbl_Dokumentpfad.text() == "Dokument vorhanden!":
                ...
                if os.path.exists("D:\DBpdf") == False:
                    os.mkdir("D:\DBpdfs")
                with tempfile.NamedTemporaryFile(suffix='.pdf', dir="D:/DBpdfs",  delete=False) as f:
                    f.write(document)
                    os.startfile(f.name)
                return
    ...
    def tW_Datenbank_Anzeigen(self):
        ...
        if self.cB_Ausbildungsthema.currentText() != "" and self.cB_Verantwortlicher.currentText() == "":
             sql = "select * from kat_lehrstoff WHERE Ausbildungsthema = '" + self.cB_Ausbildungsthema.currentText() + "'"
        elif self.cB_Verantwortlicher.currentText() != "" and self.cB_Ausbildungsthema.currentText() == "":
            sql = "select * from kat_lehrstoff WHERE Verantwortlicher = '" + self.cB_Verantwortlicher.currentText() + "'"
        else:
            sql = "select * from kat_lehrstoff WHERE Verantwortlicher = '" + self.cB_Verantwortlicher.currentText() + "' AND Ausbildungsthema = '" + self.cB_Ausbildungsthema.currentText() + "'"
    ...
    def clearFields(self):
        self.cB_Ausbildungsthema.setCurrentText("")
        self.le_Lehrstoff_ID.setText("")
        self.cB_Verantwortlicher.setCurrentText("")
    ...
    def temp_reinigen(self):
        try:
            shutil.rmtree("D:\DBpdfs")
        except Exception as e:
            print(e)
