...
class Ausbilder(QDialog, Ui_Ausbilder):
    ...
    def on_tw_anzeige_cellClicked(self, row, column):
        AUSTRITTS_DATUM = datetime.datetime.strptime(self.tw_anzeige.item(row,  5).text(),  "%d.%m.%Y")
        EINTRITTS_DATUM = datetime.datetime.strptime(self.tw_anzeige.item(row,  4).text(),  "%d.%m.%Y")
        self.le_Familienname.setText(self.tw_anzeige.item(row, 0).text())
        self.le_Vorname.setText(self.tw_anzeige.item(row, 1).text())
        self.le_Telefon.setText(self.tw_anzeige.item(row, 2).text())
        self.sb_Wochenstunden.setValue(int(self.tw_anzeige.item(row, 3).text()))
        self.de_Eintritt.setDate(EINTRITTS_DATUM)
        self.de_Austritt.setDate(AUSTRITTS_DATUM)
        self.cb_montag.setChecked(self.tw_anzeige.item(row,  6).checkState())
        self.cb_Dienstag.setChecked(self.tw_anzeige.item(row,  7).checkState())
        self.cb_Mittwoch.setChecked(self.tw_anzeige.item(row,  8).checkState())
        self.cb_Donnerstag.setChecked(self.tw_anzeige.item(row,  9).checkState())
        self.cb_Freitag.setChecked(self.tw_anzeige.item(row,  10).checkState())
    def cell_tw_anzeige(self):
        ...
        self.tw_anzeige.setColumnCount(13)
        ...
        sql = "SELECT Familienname, Vorname, Telefon, Wochenstunden, Eintrittsdatum, Austrittsdatum, wt_mo, wt_di, wt_mi, wt_do, wt_fr, Benutzer, Änderung FROM kat_ausbilder"
        mycursor = db.select(sql)
        if mycursor == "backtologin":
            self.back_to_login()
            return
    ...
    def on_pb_Hilfe_clicked(self):
        ...
        messageBox2.setWindowTitle("Fehler")
        messageBox2.setIcon(QMessageBox.Warning)
        messageBox2.setText("Diese Funktion wird in kürze hinzugefügt!")
        messageBox2.addButton("Okay", QMessageBox.NoRole) 
        messageBox2.exec_()# info asugabe 
    @pyqtSlot()
