...
import tempfile
import os
from sub.wochen_unterricht.Ui_hinzufuegen_Plan import Ui_Hinzufuegen
...
class hinzufügen(QDialog, Ui_Hinzufuegen):
    """
...
Kontrollieren Sie bitte Ihre Datenbankverbindung !"""),
    ...
    def on_comboBox_Unterrichtsthema_currentTextChanged(self, p0):
       ...
       # self.te_vorschau.setText(self.te_vorschau.text()+"     "+self.comboBox_Unterrichtsthema.currentText())
        ...
        self.comboBox_Thema1.clear()
        self.comboBox_Thema2.clear()
        self.comboBox_Thema3.clear()
        self.Themen1()
        self.Themen2()
        self.Themen3()
...
#        print(gesamt)
    ...
    def on_pushButton_hinzufuegen_clicked(self):
        ...
        if mycursor == "backtologin":
            self.back_to_login()
            return
        datum1, datum = self.Wochen_Tag.text().split(" ")
        ...
        if satz_len == 0:
           ...
           # self.INFO("Eintrag wurde in die Datenbank gespeichert!",  "I")# info asugab
            ...
            sql = "UPDATE kat_wochen_unterricht set von_bis=%s, Unterrichtsthema=%s, Ausbilder=%s, Platz_Raum=%s,Klassen=%s,Lehrlinge=%s,Informationen=%s,Ausbildungsthema_1=%s,Ausbildungsthema_2=%s,Ausbildungsthema_3=%s,Benutzer=%s,Aenderung=%s, Datum=%s WHERE Woche = '" + self.parent().label_jahr.text()+self.parent().te_KW.text() + "' and Tag='"+str(spalte)+"' and Zeile='"+self.Row.text()+"'"
...
#            print(self.comboBox_Thema1.currentText())
    ...
    def DB_Anwesenheit(self):
        self.Ermittelung_der_Nummer_Des_Tages()
        db = Database.get_instance(self)
        ID_Woche=self.parent().label_jahr.text()+self.parent().te_KW.text() +"-"+str(spalte)+"-"+self.Row.text()
        sql = "select * from kat_anwesenheit_abwesenheit WHERE ID_Woche = '" +ID_Woche+ "'"
        mycursor = db.select(sql)
        ...
        #vorschau= self.te_vorschau.item(0, 0).text()+"\n "+self.te_vorschau.item(1, 0).text()+"\n "+ self.te_vorschau.item(2, 0).text()+"\n "+self.te_vorschau.item(3, 0).text()+"\n "+self.te_vorschau.item(4, 0).text()
        Name=[]
        Name= str(self.te_vorschau.item(4,  0).text()).split("-")
        print("----"+str(Name)+"---")
        Name_len=len(Name)
        print("LENLENLENLEN-----"+str(Name_len)+"-------LENLENLENLEN")
        for a in range(len(Name)):
            print("1")
            if satz_len == 0:
                
                    #+self.Wochen_Tag.text()+") \
                sql = "INSERT INTO kat_anwesenheit_abwesenheit (ID_Woche, Name)  VALUES (%s,%s)"
                val =(ID_Woche, Name[a])
                db.insert(sql,  val)
               # self.INFO("Eintrag wurde in die Datenbank gespeichert!",  "I")# info asugab
                
            else:
                sql = "UPDATE kat_anwesenheit_abwesenheit set ID_Woche=%s, Name=%s WHERE ID_Woche = '" + ID_Woche+"'"
                #sql = "UPDATE kat_ausbilder set "+self.Wochen_Tag.text()+"=%s where KW ='" + self.parent().te_KW.text()+ "'"
                val = (ID_Woche, Name[a])
                # self.INFO("Eintrag wurde in der Datenbank aktualisiert!",  "I")# info asugabe 
    #            print("Unterricht: "+self.comboBox_Unterrichtsthema.currentText())
    ...
    def Gruppen_Anwesenheit(self):
        self.Ermittelung_der_Nummer_Des_Tages()
        db = Database.get_instance(self)
        ID_Woche=self.parent().label_jahr.text()+self.parent().te_KW.text() +"-"+str(spalte)+"-"+self.Row.text()
        sql = "select Klassen from kat_wochen_unterricht WHERE ID_Woche = '" +ID_Woche+ "'"
        mycursor = db.select(sql)
        ...
        if Gruppen.find("APP:1A") != -1:
            sql = "select Vorname, Nachname from kat_lehrlinge WHERE Gruppe = '1A' AND Lehrberuf='APP'"
        if Gruppen.find("APP:1A 1B") != -1:
            sql = "select Vorname, Nachname from kat_lehrlinge WHERE Gruppe = '1A' AND Gruppe='1B' AND Lehrberuf='APP'"
        elif Gruppen.find("APP:1A 1B 2A") != -1:
            sql = "select Vorname, Nachname from kat_lehrlinge WHERE Gruppe = '1A' AND Gruppe='1B'AND Gruppe='2A' AND Lehrberuf='APP'"
        elif Gruppen.find("APP:1A 1B 2A 2B") != -1:
            sql = "select Vorname, Nachname from kat_lehrlinge WHERE Gruppe = '1A' AND Gruppe='1B'AND Gruppe='2A'AND Gruppe='2B' AND Lehrberuf='APP'"
        elif Gruppen.find("APP:1A 1B 2A 2B 3A") != -1:
            sql = "select Vorname, Nachname from kat_lehrlinge WHERE Gruppe = '1A' AND Gruppe='1B'AND Gruppe='2A'AND Gruppe='2B'AND Gruppe='3A' AND Lehrberuf='APP'"
        elif Gruppen.find("APP:1A 1B 2A 2B 3A 3B") != -1:
            sql = "select Vorname, Nachname from kat_lehrlinge WHERE Gruppe = '1A' AND Gruppe='1B'AND Gruppe='2A'AND Gruppe='2B'AND Gruppe='3A'AND Gruppe='3B' AND Lehrberuf='APP'"
        elif Gruppen.find("APP:1A 1B 2A 2B 3A 3B 4A") != -1:
            sql = "select Vorname, Nachname from kat_lehrlinge WHERE Gruppe = '1A' AND Gruppe='1B'AND Gruppe='2A'AND Gruppe='2B'AND Gruppe='3A'AND Gruppe='3B'AND Gruppe='4A' AND Lehrberuf='APP'"
        elif Gruppen.find("APP:1A 1B 2A 2B 3A 3B 4A 4B") != -1:
            sql = "select Vorname, Nachname from kat_lehrlinge WHERE Gruppe = '1A' AND Gruppe='1B'AND Gruppe='2A'AND Gruppe='2B'AND Gruppe='3A'AND Gruppe='3B'AND Gruppe='4A'AND Gruppe='4B' AND Lehrberuf='APP'"
    ...
    def Ermittelung_der_Nummer_Des_Tages(self):
        global spalte
        if self.Wochen_Tag.text().find("Montag") != -1:
            spalte=1
        if self.Wochen_Tag.text().find("Dienstag") != -1:
            spalte=2        
        if self.Wochen_Tag.text().find("Mittwoch") != -1: 
            spalte=3        
        if self.Wochen_Tag.text().find("Donnerstag") != -1: 
            spalte=4        
        if self.Wochen_Tag.text().find("Freitag") != -1: 
            spalte=5
...
#    
    ...
    def on_pushButton_Anwesenheit_clicked(self):
        ...
        db = Database.get_instance(self)
        ID_Woche=self.parent().label_jahr.text()+self.parent().te_KW.text() +"-"+str(spalte)+"-"+self.Row.text()
        sql = "select * from Kat_lehrlinge WHERE ID_Woche = '" +ID_Woche+ "'"
        mycursor = db.select(sql)
    ...
    @pyqtSlot()
    def on_pushButton_Anzeigen_Thema1_clicked(self):
        """
        Slot documentation goes here.
        """
        db = Database.get_instance(self)
        split1,  split2= self.comboBox_Thema1.currentText().split("-")
        sql = "select Dokument from kat_lehrstoff WHERE Lehrstoff = '" + split2+ "'"
        mycursor = db.select(sql)
        try:
...
#        document = mycursor.fetchall()
    ...
    @pyqtSlot()
    def on_pushButton_Anzeigen_Thema2_clicked(self):
        """
        Slot documentation goes here.
        """
        db = Database.get_instance(self)
        split1,  split2= self.comboBox_Thema2.currentText().split("-")
        sql = "select Dokument from kat_lehrstoff WHERE Lehrstoff = '" + split2+ "'"
        mycursor = db.select(sql)
        try:
            document = mycursor.fetchone()[0]
...
#        document = mycursor.fetchall()
    ...
    @pyqtSlot()
    def on_pushButton_Anzeigen_Thema3_clicked(self):
        """
        Slot documentation goes here.
        """
        db = Database.get_instance(self)
        split1,  split2= self.comboBox_Thema3.currentText().split("-")
        sql = "select Dokument from kat_lehrstoff WHERE Lehrstoff = '" + split2+ "'"
        mycursor = db.select(sql)
        try:
            document = mycursor.fetchone()[0]
