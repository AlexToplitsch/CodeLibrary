...
from sub.wochen_unterricht.Bemerkung import Bemerkung
...
#       
    ...
    def on_pb_report_clicked(self):
        try:
            ...
            db.writeSQLite("kat_wochen_unterricht_fussbemerkungen", "Woche='"+self.label_jahr.text()+self.te_KW.text()+"'")
...
#            
            ...
            if satz_len != 0:
                sql = "Delete from kat_wochen_unterricht WHERE Woche= '" + self.label_jahr.text()+self.te_KW.text() + "' and Tag='"+str(i)+"' and von_bis=' '"
                db.delete(sql)
    ...
    def Leere_Tage_befuellen(self):
        ...
        for i in range(1, 6):
            print(i)
            j= i-1
            datum1, datum = self.tableWidget_Wochen_Plan.horizontalHeaderItem(j).text().split(" ")
            print("Spalte: "+str(i))
            ...
            if satz_len == 0:
                ...
                sql = "INSERT INTO kat_wochen_unterricht (Woche, Tag, Zeile, von_bis,Datum)  VALUES (%s,%s,%s,%s,%s)"
                val =(self.label_jahr.text()+self.te_KW.text(),i,row, leer, datum)
    ...
    @pyqtSlot()
    def on_pushButton_Bemerkung_clicked(self):
        """
        Slot documentation goes here.
        """
        w = Bemerkung(self)
        w.show()#Öffnen das SettingGui
        w.KW_l.setText(self.label_jahr.text()+self.te_KW.text())
        self.setWindowOpacity(0.0)#hide the current window
    
    @pyqtSlot(int, int)
    def on_tableWidget_Wochen_Plan_cellDoubleClicked(self, row, column):
        """
        Slot documentation goes here.
...
#            try:
    ...
    @pyqtSlot()
    def on_pushButton_daten_export_clicked(self):
        """
        Slot documentation goes here.
        """
        self.Leere_Tage_befuellen()
        self.on_pb_report_clicked()
#        self.on_pb_report_clicked_Fuß()
