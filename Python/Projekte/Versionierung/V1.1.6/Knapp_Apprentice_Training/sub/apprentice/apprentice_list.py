...
class GuiApprenticeList(QDialog, Ui_ApprenticeList):
    ...
    def db_auslesen(self):
        ...
        if  suche == "":
            ...
            sql = "SELECT * FROM kat_lehrlinge WHERE lehrbeginn >= '"+ str(diffdate1) +"' and lehrbeginn <='"+ str(diffdate2) +"'"
            cursor = db.select(sql)
            
        else:
            
            sql = "SELECT * FROM kat_lehrlinge WHERE "+ filter +" LIKE '%"+ str(suche) +"%'"
            cursor = db.select(sql)
        ...
        for z in cursor: #Zeile für Zeile auslesen und ausgeben
            ...
            lehrberuf = QTableWidgetItem(str(z[5]))
