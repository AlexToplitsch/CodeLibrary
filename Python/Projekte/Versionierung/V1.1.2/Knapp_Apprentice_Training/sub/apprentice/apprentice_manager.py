...
class GuiApprenticeManager(QDialog, Ui_ApprenticeManager):
    ...
    def stammdaten_db_auslesen(self):
        ...
        sql = "SELECT * FROM kat_list WHERE Personalnummer = '"+ self.personalnummerweitergabe +"'"
    ...
    def on_pushButton_lehrling_loeschen_clicked(self):
        ...
        if messageBox.clickedButton() == ok:
            ...
            #Lehrling löschen
            sql = "DELETE FROM kat_list WHERE Personalnummer ="+ self.personalnummerweitergabe +""
            error = db.delete(sql)
