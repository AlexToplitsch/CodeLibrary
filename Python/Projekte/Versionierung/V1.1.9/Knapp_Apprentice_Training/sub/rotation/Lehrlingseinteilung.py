...
#---------------------------------------------------------------------------------------datentabelle befüllen-------------------------------------------------------------------------
        ...
        if satz_len == 0:
            try:
                ...
                sql = "INSERT INTO kat_einteilung (Lehrling, Personalnummer, Beruf, Lehrjahr, Berufsschule, Ausbildungsjahr" + self.sqlBefehl +" ) VALUES (%s, %s, %s, %s, %s, %s"+ self.sqlBefehl+")" 
                val = self.setPermaVal()
                for i in range(len(self.val)): #die fixen werte werden mit den Ausbildungswochen-Werte kombiniert
            ...
            try:
                ...
                sql = "UPDATE kat_einteilung set (" + self.sqlBefehl + " ) VALUES (" + self.sqlBefehl +" ) WHERE Personalnummer = '" + str(self.cB_Lehrling.itemData(self.cB_Lehrling.currentIndex())) + '" AND Ausbildungsjahr = "' + self.lbl_Ausbildungjahr.text() + "'"
    ...
    def buildSQLBefehl(self):
        ...
        """
        if self.sqlBefehl == "":
            sql_ausbildungswochen = "AW1"
        else: 
            sql_ausbildungswochen = ""
            
        if self.sqlBefehlVal == "":
            sql_values = "%s"
        else: 
            sql_values = ""
        
        if self.sqlBefehl != "" and self.sqlBefehlVal != "":
            for i in range(len(self.sqlBefehl)):
                if self.sqlBefehl[i] == self.aw:
                    self.INFO("Diese Kalenderwoche wurde schon ausgeählt! Fortfahren? J/N",  "H") #das vielleicht noch mit QMesageBox wenn ja AW [i] und val[i] überschreiben
                    return
