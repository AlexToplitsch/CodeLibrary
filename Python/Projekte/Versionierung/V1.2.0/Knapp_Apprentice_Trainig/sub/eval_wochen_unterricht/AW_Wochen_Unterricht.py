...
#        self.tw_Table.setHorizontalHeaderItem(2,  colname)
    ...
    def on_btn_excel_clicked(self):
        ...
        if x == "" or y =="":
            self.INFO("Bitte wählen Sie einen Ausbilder und ein Unterrichtsthema aus.",  "I")
        else: 
            try:
                db = Database.get_instance(self)
                DBtoXML.MakeXML(db, "kat_wochen_unterricht", "DATUM >= '" + self.de_von.text() + "' AND DATUM <= '" + self.de_bis.text() + "' AND ausbilder = '" + self.cb_ausbilder.currentText() + "' AND unterrichtsthema = '"+ self.cb_unterrichtsthema.currentText() +"'", 
