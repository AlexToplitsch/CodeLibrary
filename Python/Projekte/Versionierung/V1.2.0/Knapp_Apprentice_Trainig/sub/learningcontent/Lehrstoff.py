...
class Lehrstoff(QDialog, Ui_Dialog):
    ...
    def on_pB_Speichern_clicked(self):
        ...
        if satz_len == 0:
            ...
            try:    
                ...
                val = (
                self.le_Hauptthema.text(), self.le_Lehrstoff.text(), self.cB_Verantwortlicher.currentText(), self.le_Dokument.text(),  self.sB_Stunden.value(), self.le_Hilfsmittel.text(),  self.cB_Reference.itemData(self.cB_Reference.currentIndex()), self.parent().label_eingeloggt_als.text())
    ...
    def on_tW_Datenbank_cellClicked(self, row, column):
        ...
        try:
            ...
            self.cB_Reference.setCurrentText(self.tW_Datenbank.item(row, 8).text())
        except Exception:
            self.tW_Datenbank_Anzeigen()
    ...
    def referenz_cB_befüllen(self):
        ...
        if self.cB_Filter.currentText() == "Alle Anzeigen":
            sql = "SELECT Fachrichtung, Themen_ID, Thema FROM kat_lap_themenkatalog"
        else: 
            sql = "SELECT Fachrichtung, Themen_ID, Thema FROM kat_lap_themenkatalog WHERE Fachrichtung = '" +self.cB_Filter.currentText()+"'"              #dazu noch eine on_current-textChanged (von der neuen CB) funktion wo die referenz_cb_befüllen methode aufgerufen wird
        mycursor = db.select(sql)
        rows = mycursor.fetchall()
        if mycursor == "backtologin":
            self.back_to_login()
    ...
    def tW_Datenbank_Anzeigen(self, filter1 = "",  filter2 = ""):
        ...
        if filter2 == "Alle Anzeigen":
            filter2 = ""
        if filter2 != "":
            sql =  "select Ausbildungsthema,Lehrstoff_ID,Hauptthema,Lehrstoff,Verantwortlicher,Dokument,Stunden,Hilfsmittel,LAPThemenID,Benutzer,`Änderung` from kat_lehrstoff WHERE "+ filter1 +" = '" + filter2 + "'"
        else:
