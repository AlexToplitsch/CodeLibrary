...
class Unterrichtsthemen(QDialog, Ui_Unterrichtsthemen):
    ...
    def on_tw_anzeige_cellClicked(self, row, column):
        self.le_name.setText(self.tw_anzeige.item(row, 0).text())
        self.cb_ausbilder.setCurrentText(self.tw_anzeige.item(row, 1).text())
    @pyqtSlot()
    ...
    def on_pb_excel_clicked(self):
        try:
            ...
            DBtoXML.MakeXML(db, "kat_ausbildungsthemen")
            self.setCursor(Qt.WaitCursor)
            time.sleep(2)
            self.setCursor(Qt.ArrowCursor)
            os.startfile("Excel\kat_ausbildungsthemen.xlsx")
    ...
    def on_pb_report_clicked(self):
        try:
            ...
            input_file = 'Reports/kat_ausbildungsthemen.jasper'
            output = 'Reports/kat_ausbildungsthemen'
            jasper = JasperPy()
            con={
            'driver':'generic',
            ...
            os.startfile("Reports\kat_ausbildungsthemen.pdf")
...
#        print("nach RowCount 1")
        sql = "SELECT Ausbildungsthema, Ausbilder, Benutzername, Aenderungsdatum FROM kat_ausbildungsthemen"
#        print("nach Select")
        mycursor = db.select(sql)
        #mycursor.fetchall()
