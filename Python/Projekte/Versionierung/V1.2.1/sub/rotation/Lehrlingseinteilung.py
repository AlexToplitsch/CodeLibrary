...
#---------------------------------------------------------------------------------------datentabelle befüllen-------------------------------------------------------------------------
    ...
    def buildSQLBefehl(self):
        ...
        for i in range(len(self.sqlBefehl)):
            if self.sqlBefehl[i] == str(self.aw):
                res = QMessageBox.question(
                    self,
                    self.tr(""),
                    self.tr("Die ausgewählte Kalenderwoche: " + str(self.aw) +" mit der zugehörigen Dauer: "+ str(self.sB_Dauer.value()) +" überschneidet sich mit einem bereits hinzugefügtem Eintrag. ÜBERSCHREIBEN?"),
        ...
        for i in range(self.aw,  self.aw + self.sB_Dauer.value()):
            if i == self.aw + self.sB_Dauer.value() - 1:
