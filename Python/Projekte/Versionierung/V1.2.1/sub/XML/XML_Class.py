...
class DBtoXML:
    ...
    def MakeXML(db, table, filtered="", spalten="*", filename=""):
        ...
        if filtered == "":
            ...
            return
        print(filtered)
        records="SELECT " + spalten + " FROM " + table + " WHERE " + filtered
        print(records)
        ...
        #listOfNames = [column[0] for column in db.select(columns).fetchall()]
        listOfNames =[x.strip() for x in spalten.split(',')]
        print(listOfNames)
        tree = ET.ElementTree("tree")
        top = Element('root')
        for row in rows:
        ...
        tree._setroot(top)
        if filename != "":
            tree.write("XMLFiles/" + filename + ".xml",  encoding= "UTF-8",  xml_declaration=True)
        else:
            tree.write("XMLFiles/" + table + ".xml",  encoding= "UTF-8",  xml_declaration=True)
