...
#        sql= "SELECT Klassen from kat_wochen_unterricht where Woche='"+self.parent().parent().label_jahr.text()+self.parent().parent().te_KW.text()+"'and Tag='"+str(spalte)+"'and Zeile='"+self.parent().Row.text()+"'" 
#        mycursor = db.select(sql)
#        for i in mycursor : 
#            
#            print(test)
#
#
#            mycursor = db.select("SELECT Name,Anwesend,AbwesenheitsGrund FROM kat_lehrlinge where Gruppe='"+str(i[i][1])+"'")
#            self.tableWidget.setRowCount(3) #konstante Anzahl von Zeilen in Tabelle
#            zeile = 0
#
#            for i in mycursor : 
...
# 
    ...
    def on_pb_report_clicked(self):
        try:
            db=Database.get_instance(self)
            db.writeSQLite("kat_anwesenheit_abwesenheit", "ID_Woche='"+self.parent().parent().label_jahr.text()+self.parent().parent().te_KW.text()+"-"+str(spalte)+"-"+self.parent().Row.text()+"'")
            input_file='Reports/kat_Anwesenheits_Liste.jasper'
            output='Reports/kat_Anwesenheits_Liste'
            jasper = JasperPy()
            con={
            'driver':'generic', 
            ...
            os.startfile("Reports\kat_Anwesenheits_Liste.pdf")
